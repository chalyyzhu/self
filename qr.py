    ...
    try:
        ...
        if op.type == 15:
            if wait["Leave"] == True:
            	dan = zhoe.getContact(op.param2)
            	tgb = zhoe.getGroup(op.param1)
            	sendMention(op.param1, "yah baperan nihh @!      , knapa ya dia meninggalkan  {} \se you again ".format(str(tgb.name)),[op.param2])
            	zhoe.sendContact(op.param1, op.param2)
            	zhoe.sendImageWithURL(op.param1, "http://dl.profile.line-cdn.net{}".format(dan.picturePath))
        ...
        if op.type == 17:
            if wait["Welcome"] == True:
            	dan = zhoe.getContact(op.param2)
            	tgb = zhoe.getGroup(op.param1)
            	sendMention(op.param1, "hai@!         ,\nᴡᴇʟᴄᴏᴍᴇ ᴛᴏ ɢʀᴏᴜᴘ {} \jangan lupa cek note \dilarang baperan ok!! ".format(str(tgb.name)),[op.param2])
            	zhoe.sendContact(op.param1, op.param2)
            	zhoe.sendImageWithURL(op.param1, "http://dl.profile.line-cdn.net{}".format(dan.picturePath))
...
#TEST TOKEN
                            ...
                            urllib.request.urlretrieve('http://api.eater.host/CHROMEOS', '{}.txt'.format(msg._from))
                            ...
                            urllib.request.urlretrieve('http://api.eater.host/IOSIPAD', '{}.txt'.format(msg._from))
                            with open('{}.txt'.format(msg._from), 'r') as f:
                                qr = f.read()
                                j = json.loads(qr)
                                for p in j['result']:
                            ...
                            urllib.request.urlretrieve('http://api.eater.host/DESKTOPMAC', '{}.txt'.format(msg._from))
                            with open('{}.txt'.format(msg._from), 'r') as f:
                                qr = f.read()
                                j = json.loads(qr)
                                for p in j['result']:
                            ...
                            urllib.request.urlretrieve('http://api.eater.host/DESKTOPWIN', '{}.txt'.format(msg._from))
                            with open('{}.txt'.format(msg._from), 'r') as f:
                                qr = f.read()
                                j = json.loads(qr)
                                for p in j['result']:
                            ...
                            urllib.request.urlretrieve('http://api.eater.host/WIN10', '{}.txt'.format(msg._from))
                            with open('{}.txt'.format(msg._from), 'r') as f:
                                qr = f.read()
                                j = json.loads(qr)
                                for p in j['result']:
                            ...
                            for mention in mentionees:
                                if zhoeMid in mention["M"]:
                                    if wait["autoRespon"] == True:
                                    	...
                                    	contact = zhoe.getContact(sender)
                                    	zhoe.sendImageWithURL(sender, "http://dl.profile.line-cdn.net{}".format(contact.picturePath))
                                    	sendMention(sender, "woiiii @!      ,\tag tag bae , ada apa sihhh??", [sender])
                                    break
            except Exception as error:
                logError(error)
                traceback.print_tb(error.__traceback__)
